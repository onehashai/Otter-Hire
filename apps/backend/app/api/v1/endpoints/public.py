    ...
    if conversation_ids is None:
        if candidate_created:
            ...
            if existing_conv:
                ...
                # No open conversation found → Create new one (edge case: all conversations closed)
                conversation_result = await _route_inbound_to_conversation(
                    db,
                    org_inbox,
                    payload,
                ...
                if conversation_result is not None:
                    ...
                    conversation_ids = (conv_id, msg_id)
                    logger.info(
                        "Inbound email created NEW conversation for existing candidate (no open conv found) "
                        "conv_id=%s candidate_id=%s resume_email=%s",
                        conv_id,
                        parsed_candidate.id,
                        extracted_email,
