...
class UserAdmin(ModelView, model=User):
    ...
    column_details_exclude_list = [User.hashed_password, User.verification_token_hash, User.invite_token_hash]
...
class OrgInboxAdmin(ModelView, model=OrgInbox):
    ...
    column_list = [OrgInbox.id, OrgInbox.inbox_address, OrgInbox.provider, OrgInbox.status, OrgInbox.verification_status, OrgInbox.created_at]
...
class OrgMembershipAdmin(ModelView, model=OrgMembership):
    ...
    column_list = [OrgMembership.id, OrgMembership.user_id, OrgMembership.org_id, OrgMembership.role, OrgMembership.status, OrgMembership.created_at]
...
class JobAdmin(ModelView, model=Job):
    ...
    column_list = [Job.id, Job.title, Job.status, Job.employment_type, Job.workplace_type, Job.country, Job.city, Job.openings, Job.created_at]
...
class JobCategoryAdmin(ModelView, model=JobCategory):
    ...
    column_list = [JobCategory.id, JobCategory.org_id, JobCategory.name, JobCategory.is_system_default, JobCategory.created_at]
...
class JobTeamMemberAdmin(ModelView, model=JobTeamMember):
    ...
    column_list = [JobTeamMember.id, JobTeamMember.job_id, JobTeamMember.user_id, JobTeamMember.role, JobTeamMember.created_at]
...
class StageAdmin(ModelView, model=Stage):
    ...
    column_list = [Stage.id, Stage.job_id, Stage.name, Stage.position, Stage.is_required, Stage.created_at]
...
class CandidateAdmin(ModelView, model=Candidate):
    ...
    column_list = [Candidate.id, Candidate.name, Candidate.email, Candidate.status, Candidate.source, Candidate.job_id, Candidate.created_at]
...
class CandidateDocumentAdmin(ModelView, model=CandidateDocument):
    ...
    column_list = [CandidateDocument.id, CandidateDocument.candidate_id, CandidateDocument.name, CandidateDocument.doc_type, CandidateDocument.mime_type, CandidateDocument.size_bytes, CandidateDocument.is_deleted, CandidateDocument.created_at]
    column_searchable_list = [CandidateDocument.name, CandidateDocument.field_key]
    column_sortable_list = [CandidateDocument.name, CandidateDocument.doc_type, CandidateDocument.created_at]
...
class JobApplicationAdmin(ModelView, model=JobApplication):
    ...
    column_list = [JobApplication.id, JobApplication.job_id, JobApplication.full_name, JobApplication.email, JobApplication.phone, JobApplication.status, JobApplication.created_at]
    column_searchable_list = [JobApplication.full_name, JobApplication.email]
    column_sortable_list = [JobApplication.full_name, JobApplication.email, JobApplication.status, JobApplication.created_at]
    column_default_sort = [(JobApplication.created_at, True)]

    form_excluded_columns = [JobApplication.answers, JobApplication.files, JobApplication.schema_snapshot]
...
class ConversationAdmin(ModelView, model=Conversation):
    ...
    column_list = [Conversation.id, Conversation.candidate_id, Conversation.subject, Conversation.channel, Conversation.status, Conversation.last_message_at, Conversation.created_at]
    column_searchable_list = [Conversation.subject]
    column_sortable_list = [Conversation.subject, Conversation.status, Conversation.last_message_at, Conversation.created_at]
...
class MessageAdmin(ModelView, model=Message):
    ...
    column_list = [Message.id, Message.conversation_id, Message.direction, Message.sender_type, Message.from_email, Message.to_email, Message.status, Message.created_at]
    ...
    form_excluded_columns = [Message.html_body, Message.provider_message_id, Message.email_message_id, Message.in_reply_to]
...
class EmailAdmin(ModelView, model=Email):
    ...
    column_list = [Email.id, Email.candidate_id, Email.to_email, Email.subject, Email.direction, Email.status, Email.created_at]
...
class InboundEmailAdmin(ModelView, model=InboundEmail):
    ...
    column_list = [InboundEmail.id, InboundEmail.inbox_address, InboundEmail.from_email, InboundEmail.subject, InboundEmail.email_kind, InboundEmail.parse_status, InboundEmail.received_at]
    column_searchable_list = [InboundEmail.from_email, InboundEmail.subject, InboundEmail.inbox_address]
    column_sortable_list = [InboundEmail.inbox_address, InboundEmail.parse_status, InboundEmail.received_at]
...
class InboundEmailAttachmentAdmin(ModelView, model=InboundEmailAttachment):
    ...
    column_list = [InboundEmailAttachment.id, InboundEmailAttachment.inbound_email_id, InboundEmailAttachment.filename, InboundEmailAttachment.content_type, InboundEmailAttachment.size_bytes, InboundEmailAttachment.created_at]
    column_searchable_list = [InboundEmailAttachment.filename, InboundEmailAttachment.content_type]
    column_sortable_list = [InboundEmailAttachment.filename, InboundEmailAttachment.size_bytes, InboundEmailAttachment.created_at]
...
class EmailTemplateAdmin(ModelView, model=EmailTemplate):
    ...
    column_list = [EmailTemplate.id, EmailTemplate.org_id, EmailTemplate.name, EmailTemplate.subject, EmailTemplate.created_at]
...
class InterviewAdmin(ModelView, model=Interview):
    ...
    column_list = [Interview.id, Interview.candidate_id, Interview.job_id, Interview.scheduled_at, Interview.duration_minutes, Interview.meeting_link, Interview.created_at]
    column_sortable_list = [Interview.scheduled_at, Interview.duration_minutes, Interview.created_at]
...
class FeedbackAdmin(ModelView, model=Feedback):
    ...
    column_list = [Feedback.id, Feedback.interview_id, Feedback.reviewer_user_id, Feedback.rating, Feedback.decision, Feedback.created_at]
...
class ActivityAdmin(ModelView, model=Activity):
    ...
    column_list = [Activity.id, Activity.candidate_id, Activity.created_by_user_id, Activity.type, Activity.created_at]
